...
import com.android.fileexplorer.FileIconLoader.FileId;
    ...
    private class LoaderThread extends HandlerThread implements Callback {
        ...
        Handler mHandler = new Handler() {
        	public void handleMessage(Message msg) {
        		...
        		ImageHolder holder = (ImageHolder) objects.get(0);
        		Drawable icon = (Drawable) objects.get(1);
        		ImageView view = (ImageView) objects.get(2);
        ...
        public boolean handleMessage(Message msg) {
            ...
            Iterator<Entry<ImageView, FileId>> iterator = mPendingRequests.entrySet().iterator();
            while (iterator.hasNext()) {
            	Map.Entry entry = (Entry) iterator.next();
                final FileId id = (FileId) entry.getValue();
                final ImageView view = (ImageView) entry.getKey();
                final ImageHolder holder = mImageCache.get(id.mPath);
                if (holder != null && holder.state == ImageHolder.NEEDED) {
                    // Assuming atomic behavior
                    holder.state = ImageHolder.LOADING;
                    switch (id.mCategory) {
                        case Apk:
                        	holder.setImage(mContext.getResources().getDrawable(R.drawable.ic_launcher));
