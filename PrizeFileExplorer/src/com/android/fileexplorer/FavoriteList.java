...
public class FavoriteList implements FavoriteDatabaseListener {
    ...
    public void update() {
        ...
        if (c != null) {
            while (c.moveToNext()) {
            	...
            	if (location.substring(0, location.indexOf(":")).equals("0")) {
            		if (inSD != null)
            		path = inSD + location.substring(location.indexOf(":") + 1, location.length());
            	} else {
            		if (outSD != null)
            		path = outSD + location.substring(location.indexOf(":") + 1, location.length());
            	}
            	String title = c.getString(1);
            	if (title.equals("")) {
            		title = location.equals("0:") ? mContext.getString(R.string.string_phone_capacity) 
            				: mContext.getString(R.string.out_sd);
            	}
                FavoriteItem item = new FavoriteItem(c.getLong(0), title, path);
    ...
    public void onFavoriteListItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (favorite.fileInfo.IsDir) {
            ...
            activity.setCurrentPage(Util.SDCARD_TAB_INDEX);
        } else {
            try {
